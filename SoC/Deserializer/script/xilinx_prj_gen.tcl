...
#------------------------------------------------------------------------------
proc gen_prj_struct { targetFileName device } {
    # works only for nRUns == 1; for nRuns > 1 need to rewrite 'constrs' part (see below)
    # for nRuns > 1 need to redesign concept of all build system
    set nRuns 1
    set cfgDir [pwd]
...
set src_vhd [lsearch -all -inline $srcFileList $sfx_vhd]  
...
gen_prj_struct ${TARGET_FILE_NAME} ${DEVICE}
...
#--- add VHD files and make incDir
addSrcFiles ${src_vhd} ${incDir}
...
if {$DEBUG_INFO == 1} {
        ...
        puts "\[XILINX_PRJ_GEN:DEBUG\] TOP_NAME:         $TOP_NAME"
